...
CHART_QUERIES = {
    "total_revenue_kpi": """SELECT SUM(total_amount) AS y FROM public.orders WHERE {filter_clause}""",
    "total_orders_kpi": """SELECT COUNT(order_id) AS y FROM public.orders WHERE {filter_clause}""",
    "avg_order_value_kpi": """SELECT AVG(total_amount) AS y FROM public.orders WHERE {filter_clause}""",
    "bar_sales_by_country": """SELECT shipping_address_country AS x, SUM(total_amount) AS y FROM public.orders WHERE {filter_clause} GROUP BY shipping_address_country ORDER BY x ASC""",
    "pie_orders_by_status": """SELECT status AS x, COUNT(order_id) AS y FROM public.orders WHERE {filter_clause} GROUP BY status ORDER BY x ASC""",
    "grouped_bar_country_status": """SELECT agg.x, agg.grp, agg.y FROM (SELECT shipping_address_country AS x, status AS grp, COUNT(order_id) AS y FROM public.orders WHERE {filter_clause} GROUP BY shipping_address_country, status) agg JOIN (SELECT shipping_address_country AS x, COUNT(order_id) AS x_total FROM public.orders WHERE {filter_clause} GROUP BY shipping_address_country) totals ON agg.x = totals.x ORDER BY totals.x_total DESC, agg.x""",
    ...
    "geo_sales_by_country": """SELECT shipping_address_country AS x, SUM(total_amount) AS y FROM public.orders WHERE {filter_clause} GROUP BY shipping_address_country ORDER BY x ASC""",
    "line_orders_over_time": """SELECT order_date AS x, COUNT(*) AS y FROM public.orders WHERE {filter_clause} GROUP BY order_date ORDER BY x ASC""",
...
DERIVED_CTE = """"""
DERIVED_FILTER_SOURCE = "public.orders"
