...
def main():
    ...
    with tab1:
        ...
        with _mc0:
            ...
            _metric_data = run_query("total_revenue_kpi", build_filter_clause("total_revenue_kpi", _dashboard_filters))
        ...
        with _mc1:
            ...
            _metric_data = run_query("total_orders_kpi", build_filter_clause("total_orders_kpi", _dashboard_filters))
            _metric_val = float(_metric_data.iloc[0]["y"]) if len(_metric_data) > 0 else None
            if _metric_val is not None and not (isinstance(_metric_val, float) and __import__("math").isnan(_metric_val)):
                _metric_formatted = f"{_metric_val:,.0f}"
            else:
        ...
        with _mc2:
            ...
            _metric_data = run_query("avg_order_value_kpi", build_filter_clause("avg_order_value_kpi", _dashboard_filters))
            _metric_val = float(_metric_data.iloc[0]["y"]) if len(_metric_data) > 0 else None
            if _metric_val is not None and not (isinstance(_metric_val, float) and __import__("math").isnan(_metric_val)):
        ...
        chart_data = run_query("bar_sales_by_country", build_filter_clause("bar_sales_by_country", _dashboard_filters))
        chart_data = chart_data.rename(columns={'x': 'shipping_address_country', 'y': 'total_amount'})
        chart_df = chart_data
        ...
        chart_data = run_query("pie_orders_by_status", build_filter_clause("pie_orders_by_status", _dashboard_filters))
        chart_data = chart_data.rename(columns={'x': 'status', 'y': 'order_id'})
        chart_df = chart_data
        ...
        chart_data = run_query("geo_sales_by_country", build_filter_clause("geo_sales_by_country", _dashboard_filters))
        chart_data = chart_data.rename(columns={'x': 'shipping_address_country', 'y': 'total_amount'})
        chart_df = chart_data
        effective_x_type = "None" if "None" != "None" else column_types.get("shipping_address_country")
        x_encoding_suffix = ":T" if effective_x_type == "date" else (":Q" if effective_x_type == "number" else "")
    ...
    with tab2:
        ...
        chart_data = run_query("line_orders_over_time", build_filter_clause("line_orders_over_time", _dashboard_filters))
        chart_data = chart_data.rename(columns={'x': 'order_date', 'y': 'order_id'})
        chart_df = chart_data
        ...
        chart_data = run_query("area_revenue_over_time", build_filter_clause("area_revenue_over_time", _dashboard_filters))
        chart_data = chart_data.rename(columns={'x': 'order_date', 'y': 'total_amount'})
        chart_df = chart_data
        effective_x_type = "date" if "date" != "None" else column_types.get("order_date")
        x_encoding_suffix = ":T" if effective_x_type == "date" else (":Q" if effective_x_type == "number" else "")
    ...
    with tab3:
        ...
        chart_data = run_query("scatter_amount_vs_tax", build_filter_clause("scatter_amount_vs_tax", _dashboard_filters))
        chart_data = chart_data.rename(columns={'x': 'total_amount', 'y': 'tax_amount'})
        chart_df = chart_data
        ...
        chart_data = run_query("histogram_order_amounts", build_filter_clause("histogram_order_amounts", _dashboard_filters))
        chart_data = chart_data.rename(columns={'x': 'total_amount'})
        chart_df = chart_data
        effective_x_type = "None" if "None" != "None" else column_types.get("total_amount")
        x_encoding_suffix = ":T" if effective_x_type == "date" else (":Q" if effective_x_type == "number" else "")
        ...
        chart_data = run_query("box_amount_by_status", build_filter_clause("box_amount_by_status", _dashboard_filters))
        chart_data = chart_data.rename(columns={'x': 'status', 'y': 'total_amount'})
        chart_df = chart_data
        effective_x_type = "None" if "None" != "None" else column_types.get("status")
        x_encoding_suffix = ":T" if effective_x_type == "date" else (":Q" if effective_x_type == "number" else "")
        ...
        c = alt.Chart(chart_data).mark_circle().encode(
            x=alt.X("total_amount:Q", title="Total Amount"),
            y=alt.Y("tax_amount:Q", title="Tax Amount"),
            size=alt.Size("size:Q", scale=alt.Scale(range=[50, 500]), legend=alt.Legend(title="Total Amount")),
            color=alt.Color("shipping_address_state:N", legend=alt.Legend(title="Shipping Address State")),
            tooltip=["shipping_address_state", "total_amount", "tax_amount", "size"]
        )
        st.altair_chart(c, use_container_width=True)
        st.divider()
        ...
        effective_x_type = "None" if "None" != "None" else column_types.get("shipping_address_country")
        x_encoding_suffix = ":T" if effective_x_type == "date" else (":Q" if effective_x_type == "number" else "")
        # Heatmap: 2D grid with color intensity (data pre-aggregated by SQL query)
