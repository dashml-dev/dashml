...
def main():
    ...
    with tab1:
        ...
        c = alt.Chart(chart_data).mark_bar().encode(
            x=alt.X("shipping_address_country" + x_encoding_suffix, sort=list(chart_data["shipping_address_country"]), title="Shipping Address Country"),
            y=alt.Y("order_id:Q", title="Order Id"),
            color=alt.Color("status:N",
                scale=alt.Scale(range=theme_colors),
                legend=alt.Legend(title="Status")
            ),
            xOffset=alt.XOffset("status:N", sort=list(chart_data["status"].unique())),
    ...
    with tab3:
        ...
        c = alt.Chart(chart_df).mark_circle(color="#bd93f9", size=60).encode(
            x=alt.X("total_amount:Q", sort=None, title="Total Amount"),
