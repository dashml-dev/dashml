...
import altair as alt"""
    ...
    def _build_chart_query(self, chart: dict, table_ref: str) -> str:
        """Build a per-chart SQL query template with {filter_clause} placeholder.

        The {filter_clause} is filled at runtime by build_filter_clause(), which
        combines CHART_STATIC_CONDITIONS (compile-time) with dashboard filters (runtime).
        """
        ...
        # Note: {{filter_clause}} in f-string → literal {filter_clause} in output string
        # The generated app calls query.format(filter_clause=...) at runtime.
        if chart_type in ("bar", "line", "area", "pie", "geo"):
            return f"SELECT {x}, {sql_agg}({y}) AS {y} FROM {table_ref} WHERE {{filter_clause}} GROUP BY {x}{order}{limit_clause}"
        elif chart_type in ("stacked_bar", "grouped_bar"):
            return f"SELECT {x}, {group}, {sql_agg}({y}) AS {y} FROM {table_ref} WHERE {{filter_clause}} GROUP BY {x}, {group}{order}{limit_clause}"
        elif chart_type == "heatmap":
            heatmap_y = group if group else y
            return (
                f"SELECT {x}, {heatmap_y}, {sql_agg}({y}) AS {y} FROM {table_ref}"
                f" WHERE {{filter_clause}}"
                f" AND {x} IN (SELECT {x} FROM {table_ref} GROUP BY {x} ORDER BY COUNT(*) DESC LIMIT 20)"
                f" AND {heatmap_y} IN (SELECT {heatmap_y} FROM {table_ref} GROUP BY {heatmap_y} ORDER BY COUNT(*) DESC LIMIT 20)"
                f" GROUP BY {x}, {heatmap_y}{order}{limit_clause}"
            )
        elif chart_type == "scatter":
            return f"SELECT {x}, {y} FROM {table_ref} WHERE {{filter_clause}} AND {x} IS NOT NULL AND {y} IS NOT NULL ORDER BY RAND() LIMIT 5000"
        elif chart_type == "bubble":
            size_ref = size_field or y
            return f"SELECT {group}, {sql_agg}({x}) AS {x}, {sql_agg}({y}) AS {y}, {sql_agg}({size_ref}) AS {size_ref} FROM {table_ref} WHERE {{filter_clause}} GROUP BY {group}{order}{limit_clause}"
        elif chart_type == "histogram":
            return f"SELECT {x} FROM {table_ref} WHERE {{filter_clause}} AND {x} IS NOT NULL ORDER BY RAND() LIMIT 50000"
        elif chart_type == "box":
            top_n = f"{x} IN (SELECT {x} FROM {table_ref} GROUP BY {x} ORDER BY COUNT(*) DESC LIMIT 20)"
            return f"SELECT {x}, {y} FROM {table_ref} WHERE {{filter_clause}} AND {top_n} AND {x} IS NOT NULL AND {y} IS NOT NULL ORDER BY RAND() LIMIT 50000"
    ...
    def _generate_chart_queries_dict(self, pages: list, table_ref: str) -> str:
        ...
        for page in pages:
            ...
            for chart in page["charts"]:
                queries[chart["id"]] = self._build_chart_query(chart, table_ref)
                static_conditions[chart["id"]] = self._build_static_conditions_list(
                    chart.get("filters", [])
                )
